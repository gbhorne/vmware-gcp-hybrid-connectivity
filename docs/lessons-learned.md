...
Initial troubleshooting focused on bandwidth capacity and firewall rules, delaying root cause identification by several days. Engineering team spent significant effort investigating application-layer issues before discovering the network-layer problem.

### Root Cause

VPN tunnel MTU limitation of 1460 bytes caused fragmentation of 9000 byte storage frames. Path MTU Discovery (PMTUD) was blocked by corporate firewall ICMP filtering policies, resulting in silent packet drops. Applications expecting 9000 byte MTU experienced timeouts and TCP retransmissions. Storage replication protocol interpreted packet loss as network instability, triggering exponential backoff algorithms that further reduced throughput.
...
- **Design Phase**: Document MTU through entire path (on-prem to VPN to GCP)
...
- **Validation**: Post-migration verification that no fragmentation is occurring

### Command Reference

```bash
# Test MTU with do-not-fragment flag
ping -M do -s 1432 <destination_ip>  # 1432 + 8 (ICMP) + 20 (IP) = 1460
...
Network captures showed traffic arriving at the on-premises VPN device but not appearing in GCP VPC flow logs.

### Root Cause

On-premises edge router interface was configured as access port instead of trunk port. VLAN tags were stripped before reaching the VPN tunnel, causing GCP to receive all traffic as untagged default VLAN. BGP correctly advertised routes (control plane working), but data plane traffic failed due to Layer 2 misconfiguration. The configuration error occurred during router maintenance 6 months prior when the interface was temporarily converted to access mode for testing and never reverted.
...
Traffic from GCP to on-premises worked correctly, but return traffic failed intermittently (approximately 30% packet loss). Stateful firewall logs showed dropped sessions with "no connection state" errors. Issue occurred only for certain workloads and varied by time of day, making consistent reproduction difficult.

### Root Cause

Active-active VPN tunnel configuration with unequal BGP metrics caused traffic to prefer Tunnel 1 outbound but return via Tunnel 2. The organization had two data centers with firewalls in different geographic locations:
...
Firewall A established connection state for outbound traffic, but return traffic arrived at Firewall B where no session state existed. Stateful inspection dropped return packets as "invalid connection". Time-of-day variation occurred because BGP path selection occasionally changed based on link utilization.
...
Planned on-premises router maintenance caused complete GCP connectivity loss despite redundant VPN tunnel design. BGP sessions flapped repeatedly during a 30-minute maintenance window. Connectivity restored only after maintenance completion, not during failover to secondary router as designed.

### Root Cause

BGP graceful restart was not configured on Cloud Router. When the on-premises primary router went offline for maintenance, BGP sessions terminated immediately. Cloud Router withdrew all learned routes before secondary tunnels could converge (60-90 seconds). Application servers lost connectivity during BGP convergence, triggering automated failover procedures that compounded the problem.
...
Three months into a cloud migration program, a critical IP overlap was discovered between GCP subnet 10.50.0.0/16 and an on-premises shadow IT network using the same range. The overlap prevented connectivity to mission-critical financial reporting applications not documented in the corporate IPAM system.

### Root Cause

A shadow IT department deployed financial applications using IP space not registered in corporate IPAM. Incomplete network discovery during migration planning failed to identify undocumented networks. GCP VPC was provisioned from supposedly available address space that was actually in use by production workloads. The shadow IT network had existed for 2+ years on standalone infrastructure outside standard change control.
...
**Theme 1: Testing is Never Comprehensive Enough** — Issues are discovered in production that testing should have caught. Expand test scenarios beyond the happy path to include edge cases and failure modes.

**Theme 2: Documentation Prevents Repeated Mistakes** — The same issues occur across multiple teams or deployments. Maintain a centralized lessons learned repository with search capability.

**Theme 3: Automation Reduces Human Error** — Manual configuration errors occur during repetitive tasks. Use infrastructure-as-code for all network configurations.

**Theme 4: Monitoring Enables Faster Resolution** — Extended troubleshooting results from lack of visibility. Implement comprehensive monitoring from day one, not after problems occur.

**Theme 5: Change Control Coordination is Critical** — Multi-team changes cause unexpected interactions. Assign a single change coordinator role with cross-team communication authority.

---

## Recommendations for Future Deployments

1. Spend 2-3 weeks on thorough network discovery before design
2. Build a test lab replicating production configuration for pre-deployment validation
3. Enable all monitoring before going live — don't wait for problems
4. Document architecture, configurations, decisions, and runbooks
5. Validate failover behavior before production cutover
6. Maintain step-by-step runbooks for common tasks and incidents
7. Practice deployments and rollbacks before production windows
8. Always have documented rollback procedures ready
9. Review lessons learned from similar deployments
10. Document your own lessons for future teams
